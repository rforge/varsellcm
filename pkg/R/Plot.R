...
varsellcm.plot.cont.boxplot <- function(df, y){
...
setMethod(
  ...
  definition = function(x, y, type){
    ...
    if (type=="cdf")
      ...
      varsellcm.plot.cont.boxplot(data.frame(x = x@data@data[, which(colnames(x@data@data)==y)], 
    ...
    if (x@data@withContinuous){
      if (y %in% rownames(x@param@paramContinuous@mu)){
        ...
        if (type=="cdf")
          ...
          varsellcm.plot.cont.boxplot(data.frame(x = x@data@dataContinuous@data[, which(rownames(x@param@paramContinuous@mu)==y)], 
                                                 class=as.factor(x@partitions@zMAP)),
                                      y)
        else
          stop("type must be cdf or boxplot")
