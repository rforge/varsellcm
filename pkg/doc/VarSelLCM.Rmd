---
title: '**VarSelLCM**'
subtitle: "Variable Selection for Model-Based Clustering of Mixed-Type Data Set with Missing Values."
...
* a mixed-type data set where n=270 observations are descried by d=12 features.


# Continuous data set with more features than observations

This section performs the whole analysis of the *Golub* data set.
Clustering is performed with variable selection. 
Model selection is done with MICL because n<<d.
...
To evaluated the quality of the resuling partition, we compare the true partition and its estimator given by the model
```{r, comment=""}
# Summary of the best model
