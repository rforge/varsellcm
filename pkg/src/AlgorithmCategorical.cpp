...
double AlgorithmCategorical::IntegreOneVariableCategoricalDiscrim(const int & j){
  ...
  for (int h=0; h<mj;h++){
    for (int i=0; i<data_p->m_whotakewhat[j][h].n_rows; i++)
      nkjh(m_zCandCurrent(data_p->m_whotakewhat[j][h](i)), h)+=data_p->m_w(data_p->m_whotakewhat[j][h](i));
  }
...
AlgorithmCategorical::AlgorithmCategorical(const DataCategorical * data, const int & g, const int & nbinit){
  data_p = data;
  InitCommumParamXEM(g, nbinit, data_p->m_nprofiles, data_p->m_ncols);
